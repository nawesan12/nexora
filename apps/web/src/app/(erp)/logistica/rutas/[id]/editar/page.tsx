...
import { use, useEffect, useRef, useLayoutEffect } from "react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { useForm, useFieldArray } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";
import { useRuta, useUpdateRuta } from "@/hooks/queries/use-rutas";
...
export default function EditarRutaPage({
  ...
  // Pre-populate form when ruta loads
  useEffect(() => {
    if (!ruta) return;
    form.reset({
      nombre: ruta.nombre,
      ...
      dia_semana: ruta.dia_semana != null ? String(ruta.dia_semana) : "",
  ...
  const onSubmit = (data: RutaFormInput) => {
    ...
    updateMutation.mutate(
      ...
      {
        onSuccess: () => {
          router.push(`/logistica/rutas/${id}`);
        },
      },
    );
  };
  ...
  return (
    <div ref={containerRef} className="space-y-6">
      {/* Header */}
      <div className="editar-ruta-header space-y-4">
        <Link
          href={`/logistica/rutas/${id}`}
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Editar Ruta</h1>
            <p className="text-muted-foreground mt-0.5">
              Modifica los datos de la ruta &quot;{ruta.nombre}&quot;
            </p>
          </div>
        </div>
      </div>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex items-center justify-end gap-3">
              <Button
                type="button"
                variant="outline"
                onClick={() => router.push(`/logistica/rutas/${id}`)}
              >
                Cancelar
              </Button>
